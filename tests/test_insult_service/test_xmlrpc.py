import unittest
from insult_service.xmlrpc_impl.xmlrpc_server import InsultService

class TestInsultServiceXMLRPC(unittest.TestCase):
    def setUp(self):
        """Configura una nueva instancia de InsultService antes de cada prueba."""
        self.insult_service = InsultService()

    def test_add_insult(self):
        """Prueba agregar insultos al servicio."""
    ...
    def test_get_insults(self):
        ...
        self.assertSetEqual(set(result), set(insults))  # Convertimos a set para evitar problemas de orden

    def test_get_random_insult(self):
        """Prueba obtener un insulto aleatorio."""
        insults = ["You are a fool!", "You are an idiot!", "You are a nincompoop!"]
        for insult in insults:
            self.insult_service.add_insult(insult)
    ...
    def test_get_random_insult_empty(self):
        """Prueba obtener un insulto cuando la lista está vacía."""
